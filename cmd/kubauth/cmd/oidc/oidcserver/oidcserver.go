...
func NewOIDCServer(router *http.ServeMux, userDb userdb.UserDb, loginTemplate *template.Template) *OIDCServer {
	myStorage := storage.NewMemoryStore()
	...
	oauth2 := compose.ComposeAllEnabled(fositeConfig, myStorage, privateKey)
