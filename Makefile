...
HELM_KUBAUTH_SERVER_VERSION ?= 0.1.1-snapshot
HELM_KUBAUTH_GRANT_VERSION ?= 0.1.1-snapshot
...
crds: manifests kustomize ## Generate crds file into helm chart
	$(KUSTOMIZE) build config/crd -o helm/kubauth-server/crds/crds.yaml

# ----------------------

define CHART_KUBAUTH_SERVER_YAML
# File generated by Makefile
apiVersion: v2
name: kubauth-server
version: $(HELM_KUBAUTH_SERVER_VERSION)
...
endef
export CHART_KUBAUTH_SERVER_YAML

.PHONY: chart-kubauth-server-yaml
chart-kubauth-server-yaml: ## Generate the helm/kubauth-server/Chart.yaml
	echo "$$CHART_KUBAUTH_SERVER_YAML" >./helm/kubauth-server/Chart.yaml

.PHONY: chart-kubauth-server
chart-kubauth-server: chart-kubauth-server-yaml crds ## Build and push oidc server helm chart
	cd ./helm && helm package -d ./../tmp kubauth-server && helm push ./../tmp/kubauth-server-${HELM_KUBAUTH_SERVER_VERSION}.tgz oci://${HELM_DOCKER_REPO}

# ----------------------

define CHART_KUBAUTH_GRANT_YAML
# File generated by Makefile
apiVersion: v2
name: kubauth-grant
version: $(HELM_KUBAUTH_GRANT_VERSION)
appVersion: $(APP_VERSION)
description: "Kubauth, the Kubernetes authentication system"
keywords:
  - kubauth
...
maintainers:
  - name: kubauth
    url: https://github.com/kubauth
endef
export CHART_KUBAUTH_GRANT_YAML

.PHONY: chart-kubauth-grent-yaml
chart-kubauth-grant-yaml: ## Generate the helm/kubauth-grant/Chart.yaml
	echo "$$CHART_KUBAUTH_GRANT_YAML" >./helm/kubauth-grant/Chart.yaml

.PHONY: chart-kubauth-grant
chart-kubauth-grant: chart-kubauth-grant-yaml crds ## Build and push oidc grant helm chart
	cd ./helm && helm package -d ./../tmp kubauth-grant && helm push ./../tmp/kubauth-grant-${HELM_KUBAUTH_GRANT_VERSION}.tgz oci://${HELM_DOCKER_REPO}



charts: chart-kubauth-server chart-kubauth-grant	## Build all charts
##@ Dependencies
